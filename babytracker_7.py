...
if selected_tab == "Dashboard":
    ...
    if not luier_df.empty:
        ...
        nat_df = luier_df[luier_df['Type Luier'] == 'Nat']
        vuil_df = luier_df[luier_df['Type Luier'] == 'Vuil']

        nat_count = len(nat_df)
        vuil_count = len(vuil_df)

        laatste_nat = nat_df.sort_values('Starttijd', ascending=False).iloc[0]['Starttijd'].strftime('%H:%M') if nat_count else "-"
        laatste_vuil = vuil_df.sort_values('Starttijd', ascending=False).iloc[0]['Starttijd'].strftime('%H:%M') if vuil_count else "-"

        col3.markdown(
            f"**🧷 Luiers vandaag**\n\n"
            f"Nat: {nat_count} (Laatste: {laatste_nat})  \n"
            f"Vuil: {vuil_count} (Laatste: {laatste_vuil})"
        )
    else:
        col3.markdown(
            "**🧷 Luiers vandaag**\n\n"
...
# ------------------------------
    ...
    # Default: Fles, kunstvoeding, 100ml
    voeding_type = st.selectbox(
        "Type voeding", 
        ['Borst', 'Fles', 'Kolven', 'Hapje'], 
        index=1,  # Default is 'Fles'
    ...
    if voeding_type == 'Borst':
        ...
        opm = st.text_input('Opmerking', key='voeding_opm_borst')
    elif voeding_type == 'Fles':
        fles = st.selectbox('Type fles', ['melk', 'kunstvoeding'], index=1, key='voeding_fles_manual')  # default kunstvoeding
        hoeveelheid = st.number_input('Hoeveelheid (ml)', min_value=0, value=100, key='voeding_hoeveelheid_fles')  # default 100 ml
        opm = st.text_input('Opmerking', key='voeding_opm_fles')
        ...
        opm = st.text_input('Opmerking', key='voeding_opm_hapje')
...
if selected_tab == "Gezondheid":
    ...
    gewicht = st.number_input('Gewicht (gram)', min_value=0.0, step=0.1, value=5000, key='g_gewicht')
